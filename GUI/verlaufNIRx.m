...
function []= verlaufNIRx(oxy_signal, deoxy_signal, NIRx, settings, t, SollAktivierung, fs, c, data, Task, ImageClass)
...
if DisplaySteps && settings.generateFiguresRAW
    % only plotted if the channel is equal to the current iteration in
    % outer for-loop
    if kanal(c) % && NIRx.settings.createFigures
        %
        hCon = figure(400+c);
        ...
        plot(t', SollAktivierung*ax1(4),'g')
...
%             print( '-dpng', '-r300', [DataPath, Gruppe, '\', AnalysePath, '\',NIRx.settings.Subj,'_',Task,'_',ImageClass,'_Konzentrationsänderung', '_','Kanal',num2str(c),NIRx.settings.Usage.CAR,NIRx.settings.Usage.corrmode]);
            saveas(hCon, [data.analysisPath filesep data.analysisFilename '_Conc_Chg_Raw', '_','Ch',num2str(c),NIRx.settings.Usage.CAR,NIRx.settings.Usage.corrmode],'fig');
        else
%             print( '-dpng', '-r300', [DataPath, Gruppe, '\', AnalysePath, '\',NIRx.settings.Subj,'_',Task,'_',ImageClass,'_Konzentrationsänderung', '_','Kanal',num2str(c),NIRx.settings.Usage.CAR,NIRx.settings.Usage.TFICA,NIRx.settings.Usage.corrmode]);
            saveas(hCon, [data.analysisPath filesep data.analysisFilename '_Conc_Chg_Raw', '_','Ch',num2str(c),NIRx.settings.Usage.CAR,NIRx.settings.Usage.TFICA,NIRx.settings.Usage.corrmode],'fig');
        end
