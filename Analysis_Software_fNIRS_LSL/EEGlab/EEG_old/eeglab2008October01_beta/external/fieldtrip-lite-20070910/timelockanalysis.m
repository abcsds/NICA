...
% for backward compatibility, in case the raw data does not contain a time axis for every trial
if ~isfield(data, 'time')
  data.time = {};
  for i=1:ntrial
    data.time{i} = offset2time(data.offset(i), data.fsample, size(data.trial{i},2));
  end
end
...
for i=1:ntrial
  ...
  % determine whether the data in this trial can bve used for all the requested computations
  switch cfg.vartrllength
    case 0
...
cfg.version.id = '$Id: timelockanalysis.m,v 1.47 2007/05/30 11:41:10 roboos Exp $';
