...
%   cfg.normalize   = 'no' (default), or 'yes'
%
...
for i=1:length(grid.inside)
  ...
  grid.leadfield{dipindx} = compute_leadfield(grid.pos(dipindx,:), sens, vol, 'reducerank', cfg.reducerank, 'normalize', cfg.normalize);
  ...
  if isfield(cfg, 'grid') && isfield(cfg.grid, 'ori')
    ...
    grid.leadfield{dipindx} = grid.leadfield{dipindx} * grid.ori(dipindx,:)';
  end
...
cfg.version.id = '$Id: prepare_leadfield.m,v 1.20 2007/05/15 07:01:55 roboos Exp $';
