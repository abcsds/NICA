...
if ~(ischar(cfg.baseline) && strcmp(cfg.baseline, 'no'))
  ...
  elseif strcmp(timelock.dimord, 'subj_chan_time')
    ...
    nsubj = size(timelock.subj,1);
    if isfield(cfg, 'channel')
      % only apply on selected channels
      for i=1:nsubj
        timelock.subj(i,chansel,:) = blc(timelock.subj(i,chansel,:), tbeg:tend);
      end
    else
      % apply on all channels
      for i=1:nsubj
        timelock.subj(i,:,:) = blc(timelock.subj(i,:,:), tbeg:tend);
      end
    end
  end
...
cfg.version.id = '$Id: timelockbaseline.m,v 1.8 2007/06/15 10:00:42 jansch Exp $';
