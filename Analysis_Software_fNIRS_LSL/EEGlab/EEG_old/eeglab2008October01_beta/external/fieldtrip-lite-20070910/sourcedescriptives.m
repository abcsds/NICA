...
if strcmp(source.method, 'jackknife') || strcmp(source.method, 'bootstrap') || strcmp(source.method, 'pseudovalue') || strcmp(source.method, 'singletrial') || strcmp(source.method, 'rawtrial')
  ...
  sumdip = [];
  if isfield(dip(1), 'var'),    sumdip.var    = nan*zeros(size(dip(1).var  )); end
  if isfield(dip(1), 'pow'),    sumdip.pow    = nan*zeros(size(dip(1).pow  )); end
  if isfield(dip(1), 'coh'),    sumdip.coh    = nan*zeros(size(dip(1).coh  )); end
  if isfield(dip(1), 'rv'),     sumdip.rv     = nan*zeros(size(dip(1).rv   )); end
  if isfield(dip(1), 'noise'),  sumdip.noise  = nan*zeros(size(dip(1).noise)); end
  if isfield(dip(1), 'nai'),    sumdip.nai    = nan*zeros(size(dip(1).nai  )); end
  sqrdip = [];
  if isfield(dip(1), 'var'),    sqrdip.var    = nan*zeros(size(dip(1).var  )); end
  if isfield(dip(1), 'pow'),    sqrdip.pow    = nan*zeros(size(dip(1).pow  )); end
  if isfield(dip(1), 'coh'),    sqrdip.coh    = nan*zeros(size(dip(1).coh  )); end
  if isfield(dip(1), 'rv'),     sqrdip.rv     = nan*zeros(size(dip(1).rv   )); end
  if isfield(dip(1), 'noise'),  sqrdip.noise  = nan*zeros(size(dip(1).noise)); end
  if isfield(dip(1), 'nai'),    sqrdip.nai    = nan*zeros(size(dip(1).nai  )); end
...
cfg.version.id = '$Id: sourcedescriptives.m,v 1.36 2007/05/08 21:04:40 roboos Exp $';
