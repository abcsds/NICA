  ...
  switch cfg.model
    ...
    case 'moving'
      for t=1:ntime
        ...
        if cfg.numdipoles==1
          fprintf('found minimum after scanning for latency %d on grid point [%g %g %g]\n', t, dip(t).pos(1), dip(t).pos(2), dip(t).pos(3));
        elseif cfg.numdipoles==2
          fprintf('found minimum after scanning for latency %d on grid point [%g %g %g; %g %g %g]\n', t, dip(t).pos(1), dip(t).pos(2), dip(t).pos(3), dip(t).pos(4), dip(t).pos(5), dip(t).pos(6));
        end
      end
  end % switch model
end % if gridsearch
  ...
  case 'moving'
    ...
    for t=1:ntime
      ...
      try
        ...
        success(t) = 1;
        if cfg.numdipoles==1
          fprintf('found minimum after non-linear optimization for latency %d on [%g %g %g]\n', t, dipout(t).pos(1), dipout(t).pos(2), dipout(t).pos(3));
        elseif cfg.numdipoles==2
          fprintf('found minimum after non-linear optimization for latency %d on [%g %g %g; %g %g %g]\n', t, dipout(t).pos(1,1), dipout(t).pos(1,2), dipout(t).pos(1,3), dipout(t).pos(2,1), dipout(t).pos(2,2), dipout(t).pos(2,3));
        end
      catch
...
cfg.version.id = '$Id: dipolefitting.m,v 1.44 2007/04/03 15:37:07 roboos Exp $';
