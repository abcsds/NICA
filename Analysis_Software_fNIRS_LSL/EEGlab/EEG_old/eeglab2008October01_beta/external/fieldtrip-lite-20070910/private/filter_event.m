...
for i=1:length(event)
  ...
  if testsample,   sel(i) = sel(i) && (event(i).sample == sample); end
  if testoffset,   sel(i) = sel(i) && (event(i).offset == offset); end
  if testduration, sel(i) = sel(i) && (event(i).duration == duration); end
