...
if strcmp(optimfun, 'fminunc')
  ...
  options = optimset('TolFun',1e-9,...
                     ...
                     'Display',display);
  [param, fval, exitflag, output] = fminunc(@dipfit_error, param, options, dat, sens, vol, constr, metric, checkinside);
else
  % do non-linear optimization of the dipole parameters using standard Matlab fminsearch()
  options = optimset('MaxIter',maxiter,...
                     'MaxFunEvals',2*maxiter*length(param),...
                     'Display',display);
  [param, fval, exitflag, output] = fminsearch(@dipfit_error, param, options, dat, sens, vol, constr, metric, checkinside);
end
...
[err, dipout.mom] = dipfit_error(param, dat, sens, vol, constr, metric, checkinside);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [err, mom] = dipfit_error(param, dat, sens, vol, constr, metric, checkinside);
