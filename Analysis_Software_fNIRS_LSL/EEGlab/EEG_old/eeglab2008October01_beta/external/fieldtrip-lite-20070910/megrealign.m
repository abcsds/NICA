...
[vol, data.grad] = prepare_vol_sens(tmpcfg, tmpdat);
tmpdat.grad = template.grad;
[vol, template.grad] = prepare_vol_sens(tmpcfg, tmpdat);
...
grid = prepare_dipole_grid(cfg, vol, data.grad);
pos = grid.pos;
clear grid
...
lfold = compute_leadfield(pos, data.grad, vol);
lfnew = compute_leadfield(pos, template.grad, vol);
...
cfg.version.id   = '$Id: megrealign.m,v 1.44 2007/05/30 13:23:55 roboos Exp $';
