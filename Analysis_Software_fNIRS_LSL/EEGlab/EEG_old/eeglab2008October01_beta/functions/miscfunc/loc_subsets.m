...
%       >> [subsets subidx pos] = loc_subsets(chanlocs, nchans, plotobj, plotchans);
%
% Inputs:
%
...
%
%   >> subset = loc_subsets([32 60 100], EEG.chanlocs, true, true);
%
...
function [subset idx pos] = loc_subsets(chanlocs, numberOfChannelsInSubset, plotOptimization, plotSubsets);
...
for i=1:length(numberOfChannelsInSubset)
    setId{i} = r(1:numberOfChannelsInSubset(i));
    r(1:numberOfChannelsInSubset(i)) = [];
end;
        ...
        for set2 = (set1 + 1):length(setId)
            for i = 1:length(setId{set1})
                for j = 1:length(setId{set2})
                    chan(1) = setId{set1}(i);
                    chan(2) = setId{set2}(j);
                    if cost_of_exchanging_channels(chan,[set1 set2], setId, dist) < 0
