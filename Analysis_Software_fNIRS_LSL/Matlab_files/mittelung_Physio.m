...
function mittelung_Physio(NIRx, Group, DataPath, AnalysePath, Task, ImageClass, trig, BPfs, gUSBampfs)
    ...
    for k = 2 : length(trig) 
        ...
        data_ch_BP(:,1,k) = NIRx.Data.BPdia(idx + t_trial_bp,1)-mean(NIRx.Data.BPdia(idx + t_trial_bp(1:((Timing(1)*BPfs)*(-1)+1)),1));
        data_ch_BP(:,2,k) = NIRx.Data.BPsys(idx + t_trial_bp,1)-mean(NIRx.Data.BPsys(idx + t_trial_bp(1:((Timing(1)*BPfs)*(-1)+1)),1));
    ...
    %Auf ref-intervall referenzieren     
    dat_avg_BP(:,1)=dat_avg_BP(:,1)-mean(dat_avg_BP(1:(Timing(1)*BPfs)*(-1)+1,1));
    dat_avg_BP(:,2)=dat_avg_BP(:,2)-mean(dat_avg_BP(1:(Timing(1)*BPfs)*(-1)+1,2));

    figure(2)
    subplot(3,1,1)
    title(['Mean Physio Signals ', Group, ' ', ImageClass],'FontSize',12)
    ...
    print( '-dpng', '-r300', [DataPath, Group, '\', AnalysePath, '\',NIRx.settings.Subj,'_',Task,'_',ImageClass,'_Mean_Physio_Signals']);
    saveas(gcf, [DataPath, Group, '\', AnalysePath, '\',NIRx.settings.Subj,'_',Task,'_',ImageClass,'_Mean_Physio_Signals'],'fig');             
end
