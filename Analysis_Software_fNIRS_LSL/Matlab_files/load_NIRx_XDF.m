function [NIRx]=load_NIRx_XDF(D)

    j = 1;

    % Dissolve different filenames
    for i = 1 : length(D{j}.XDF)
        % get filename of NIRStar header. If only one header from NIRStar
        % available choose first one for computation
        if length(D{j}.hdr) ~= length(D{j}.XDF)
            FileName_hdr{1} = [D{j}.FileDir D{j}.hdr{:,1}];
        else
            FileName_hdr{i} = [D{j}.FileDir D{j}.hdr{:,i}];
        end
        
        % get filename of XDF
        FileName_XDF{i} = [D{j}.FileDir  D{j}.XDF{i}.FileName];

        KnownNrofTRials = D{j}.KnownNrofTrials;
    end
    clear i;
    ...
    for i = 1 : length(FileName_XDF)    %Je ein Durchlauf pro Datensatz
        ...
        if length(FileName_hdr) > 1
            disp(['Loading... ' FileName_hdr{i}])
            fid_in = fopen(FileName_hdr{i}, 'r');
        else
            disp(['Loading... ' FileName_hdr{1}])
            fid_in = fopen(FileName_hdr{1}, 'r');
        end
        % input = '';
        ...
        for j = 1 : length(data_xdf)
            switch data_xdf{j}.info.name%source_id % .name
                case 'NIRStar'%'NIRStar-TCP-Stream' %'NIRStar'
                   ...
                   disp([data_xdf{j}.info.source_id ' found in XDF. Channels: ' num2str(Channels)]);

                case 'NIRStar-Markers'%'NIRStar-TCP-Stream_markers' %'NIRStar-Markers'      
                   marker_nirx = data_xdf{j};
                   hdr.Bool.Marker = true;
                   disp([data_xdf{j}.info.source_id ' found in XDF.']);

                   if hdr.Bool.paradigm
                       disp('Paradigm trigger already found!');
                       return
                   end
                   ...
                   if mod(length(run_start_stop),2) ~= 0
                       disp('Error: Start/Stop markers uneven.');
                       return
                   end
                   
                   if (length(Markers_Class)) ~= KnownNrofTRials
                       disp('Error: found triggers do not match number of known trials!');
                       return
                   end
                ...
                case 'paradigm'
                   ...
                   if hdr.Bool.Marker
                       disp('NIRS-Marker will be overwritten by paradigm-Marker!');
                   end
                   ...
                   % trigger=1
                   run_start_stop = Markers_Time(Markers_Class(:) == 1);
    ...
    if (~isempty(gUSBamp_time))
        ...
        NIRx.Data.HR = calcHRlin(gUSBamp_ecg(:,1), round(gusbamp.info.effective_srate(1)));
