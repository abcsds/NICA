...
function r=calcNIRSspectra(signal,fs)
    ...
    [p, f] = pwelch(signal-mean(signal), window, window_overlap*round(fs), fft_length*round(fs), round(fs));
%     [p, f] = psd(signal-mean(signal), fft_length*round(fs), round(fs), window, window_overlap*round(fs),'linear');
